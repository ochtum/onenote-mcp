...
### Setup for GitHub Copilot (VS Code extension)

1. Clone this repository and follow the installation steps below
2. Create or update `.vscode/mcp.json` in this project:
   ```json
   {
     "servers": {
       "onenote": {
   ...
   ```
3. Open `.vscode/mcp.json` in VS Code and click the `Start` button shown for MCP servers
4. Open Copilot Chat, switch to `Agent` mode, and confirm the `onenote` tools are available from the tools icon
5. Ask Copilot to access OneNote in natural language

### Setup for Codex CLI (WSL)

1. Clone this repository and follow the installation steps below
2. Add this MCP server to Codex CLI from WSL:
   ```bash
...
3. Verify registration:
   ...
   codex mcp list
   ```
4. Start Codex CLI and ask OneNote-related requests; Codex will launch the MCP server process automatically
